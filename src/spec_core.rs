...
impl SpecCompute {
    ...
    /// # Arguments
    ///
    ///  * `self` -  `self` is immutable so that the FFT plans can be shared
    ///    between threads. Since the FFT plans are not thread safe, they are
    ///    wrapped in a Mutex.
    ///
    ///  * `data` -  The time domain data to compute the spectrogram from.
    ///    `data` is not preprocessed other than windowing and casting to
    ///    complex.
    ///
    #[cfg(feature = "rayon")]
